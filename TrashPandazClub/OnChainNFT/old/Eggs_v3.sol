...
contract Gen3Generator{
    function generateCharacter(string[5] memory _color, string memory _chromosome) public returns(string memory) {}
}
...
contract Eggs is ERC1155URIStorage, AccessControl{
    ...
    function _tokenURIPrefix() private pure returns (string memory) {
        return '{ "name": "TPCGen3 #';
    }
    ...
    function _tokenURIPayload(uint256 tokenId) private view returns (string memory) {
        if (tokens[tokenId].hatched == false) {
            return string(
                abi.encodePacked(
                    tokenId.toString(),
                    '", "description": "TrashPandazClub Gen3", "image": "',
    ...
    function hatch(uint _tokenId) public{
        ...
        tokens[_tokenId].image = Gen3Generator(gen3GeneratorContract).generateCharacter(tokens[_tokenId].color, tokens[_tokenId].chromosome);
    }
