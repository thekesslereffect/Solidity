...
contract TPCGen3 is ERC1155URIStorage, AccessControl, Pausable  {
    ...
    // Rarity percentages
    uint constant commonPercent = 60;
    uint constant rarePercent = 32;
    uint constant legendaryPercent = 8;
    ...
    function getLevel(uint256 tokenId) public view returns (string memory) {
        return tokenIdToLevel[tokenId].toString();
    }
    function getChromosome(uint256 tokenId) public view returns (string memory) {
        return tokenIdToChromosome[tokenId];
    }
    ...
    function generateCharacter(uint _tokenId) public view returns (string memory){
        ...
        string memory _finalImage = TPCGen3_Image2.generateCharacter(_result, tokenIdToColor1[_tokenId],tokenIdToColor2[_tokenId],tokenIdToColor3[_tokenId],tokenIdToColor4[_tokenId],tokenIdToColor5[_tokenId],getLevel(_tokenId),getChromosome(_tokenId));
        return _finalImage;
    }
    ...
    function mint() public whenNotPaused{
        ...
        tokenIdToChromosome[newItemId] = generateChromosome(_rand);
        tokenIdToRarity[newItemId] = generateRarity(_rand);
        _mint(msg.sender, newItemId, 1, "");
        generateColorByRarity(_rand, tokenIdToRarity[newItemId], newItemId);
        _setURI(newItemId, getTokenURI(newItemId));
    }

    function generateChromosome(uint _rand) private pure returns(string memory){
        if((_rand%2) == 1) {
            return "XX";
        } else {
            return "XY";
    ...
    function generateRarity(uint _rand) private pure returns(string memory){
    ...
    uint constant NUM_COLORS = 5;
    uint constant NUM_SHADES = 4;
    ...
    function generateColorByRarity(uint _rand, string memory _rarity, uint _tokenId) private {
        ...
        uint[NUM_COLORS] memory indices = [_rand % NUM_SHADES, (_rand >> 32) % NUM_SHADES, (_rand >> 64) % NUM_SHADES, (_rand >> 96) % NUM_SHADES, (_rand >> 128) % NUM_SHADES];
    ...
    function customColorForLegendary(uint _tokenId, string memory _color1, string memory _color2, string memory _color3, string memory _color4, string memory _color5) public {
        ...
        require(keccak256(abi.encodePacked(tokenIdToRarity[_tokenId])) == keccak256((abi.encodePacked("common"))), "Only Legendary TrashPandaz can be customized");
        tokenIdToColor1[_tokenId] = _color1;
        tokenIdToColor2[_tokenId] = _color2;
        tokenIdToColor3[_tokenId] = _color3;
        tokenIdToColor4[_tokenId] = _color4;
        tokenIdToColor5[_tokenId] = _color5;
    }
